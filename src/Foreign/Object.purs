...
instance traversableWithIndexObject :: TraversableWithIndex String Object where
  traverseWithIndex f ms = fold (\acc k v -> flip (insert k) <$> acc <*> f k v) (pure empty) ms
...
unionWith f m1 m2 = mutate (\s1 -> foldM (\s2 k v1 -> OST.poke k (runFn4 _lookup v1 (\v2 -> f v1 v2) k m2) s2) s1 m1) m2
